...
from layers.Transformer_EncDec import Encoder, class_Encoder
from layers.SelfAttention_Family import FullAttention, AttentionLayer
from layers.Embed import  DataEmbedding_inverted, DataEmbedding_classficaiton
...
class FFSTT_Pytorch(nn.Module):
    def __init__(self, sequence_length):
        ...
        self.encoder = Encoder(
            [class_Encoder(AttentionLayer(FullAttention(False, 7, attention_dropout=0.1,
                                      output_attention='store_true'), 64, 8),
                    64,
                    sequence_length,
                    64,
                    dropout=0.1,
...
def train(appliance_name, model, mains, appliance, epochs, batch_size, pretrain, checkpoint_interval=None, train_patience=3):
    ...
    train_mains, valid_mains, train_appliance, valid_appliance = train_test_split(mains, appliance, test_size=0.2,
    ...
    for epoch in range(epochs):
        ...
        if best_loss is None or final_loss < best_loss:
            ...
            path_state_dict = "./" + appliance_name + "_FFSTT_best_state_dict.pt"
        ...
        if (checkpoint_interval != None) and ((epoch + 1) % checkpoint_interval == 0):
            checkpoint = {"model_state_dict": model.state_dict(),
                          ...
                          "epoch": epoch}
            path_checkpoint = "./" + appliance_name + "_FFSTT_checkpoint_{}_epoch.pkl".format(epoch)
...
def transfer_fine(appliance_name, model, mains, appliance, epochs, batch_size, train_patience = 3):
    ...
    for epoch in range(epochs):
        ...
        if best_loss is None or final_loss < best_loss:
            best_loss = final_loss
            patience = 0
            best_model_path = "./{}_FFSTT_best_state_dict.pt".format(appliance_name)
            lora_path = "./{}_FFSTT_lorapoint.pt".format(appliance_name)
...
class FFSTT(Disaggregator):

    def __init__(self, params):
        self.MODEL_NAME = "FFSTT"
        self.models = OrderedDict()
        self.chunk_wise_training = params.get('chunk_wise_training', False)
        self.sequence_length = params.get('sequence_length', 699)
    ...
    def partial_fit(self, train_main, train_appliances, pretrain=False, fine_tuning=False, do_preprocessing=True, **load_kwargs):
        ...
        print("...............FFSTT partial_fit running...............")
        ...
        for appliance_name, power in train_appliances:
            if appliance_name not in self.models:
                ...
                self.models[appliance_name] = FFSTT_Pytorch(self.sequence_length)
                # Load pretrain dict or not
                if pretrain is True:
                    self.models[appliance_name].load_state_dict(
                        torch.load("./" + appliance_name + "_FFSTT_best_state_dict.pt"))
            ...
            self.models[appliance_name].load_state_dict(torch.load("./" + appliance_name + "_FFSTT_best_state_dict.pt"))
