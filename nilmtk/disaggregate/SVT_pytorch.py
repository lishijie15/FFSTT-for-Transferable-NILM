...
from layers.Transformer_EncDec import Encoder, SVT_Encoder
from layers.SelfAttention_Family import FullAttention, AttentionLayer
from layers.Embed import DataEmbedding_whole, DataEmbedding_inverted
...
class SVT_Pytorch(nn.Module):

    def __init__(self, sequence_length):
        super(SVT_Pytorch, self).__init__()
        self.seq_len = sequence_length
        # Embedding
        self.enc_embedding = DataEmbedding_inverted(sequence_length, d_model=64)
        # Encoder
        self.encoder = Encoder(
            [SVT_Encoder(AttentionLayer(FullAttention(False, 7, attention_dropout=0.1,
        ...
        self.projection = nn.Linear(64 * 1, 1)  #num_class是输出长度
...
def train(appliance_name, model, mains, appliance, epochs, batch_size, pretrain,checkpoint_interval = None,  train_patience = 3):
    ...
    for epoch in range(epochs):
        ...
        if best_loss is None or final_loss < best_loss:
            ...
            path_state_dict = "./"+appliance_name+"_SVT_best_state_dict.pt"
        ...
        if (checkpoint_interval != None) and ((epoch + 1) % checkpoint_interval == 0):
            checkpoint = {"model_state_dict": model.state_dict(),
                            ...
                            "epoch": epoch}
            path_checkpoint = "./"+appliance_name+"_SVT_checkpoint_{}_epoch.pkl".format(epoch)
    ...
    def partial_fit(self,train_main,train_appliances,pretrain = False,do_preprocessing=True,  **load_kwargs):
        ...
        for appliance_name, power in train_appliances:
            if appliance_name not in self.models:
                ...
                self.models[appliance_name] = SVT_Pytorch(self.sequence_length)
                # Load pretrain dict or not
                if pretrain is True:
                    self.models[appliance_name].load_state_dict(torch.load("./"+appliance_name+"_SVT_pre_state_dict.pt"))
            ...
            self.models[appliance_name].load_state_dict(torch.load("./"+appliance_name+"_SVT_best_state_dict.pt"))
