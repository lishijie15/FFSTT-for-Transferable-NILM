...
def transfer_fine(appliance_name, gen_model, dis_model, mains, appliance, epochs, batch_size, pretrain=False, checkpoint_interval=None, train_patience=3):
    # Model configuration
    if USE_CUDA:
        gen_model = gen_model.cuda()
        dis_model = dis_model.cuda()
    ...
    pretrained_dict = torch.load(os.path.join("./" + appliance_name + "_energan_best_state_dict_uk.pt"))
    gen = gen_model.state_dict()

    pretrained_dict = {k: v for k, v in pretrained_dict.items() if
                       k in gen and gen[k].size() == v.size()}

    # 更新当前模型的状态字典
    gen.update(pretrained_dict)
    gen_model.load_state_dict(gen, strict=False)
    ...
    def partial_fit(self, train_main, train_appliances, pretrain = False, fine_tuning=False, do_preprocessing=True, **load_kwargs):
        ...
        for appliance_name, power in train_appliances:
            ...
            if fine_tuning==False:
                ...
                transfer_fine(appliance_name, gen_model, dis_model, train_main, power, self.n_epochs, self.batch_size, train_patience=3)
