...
def test(model, test_mains, appliance_name, batch_size = 512, checkpoint_path=None):
    # Model test
    st = time.time()

    print('loading model')
    if checkpoint_path is None:
        checkpoint_path = "./" + appliance_name + "_dae_best_state_dict.pt"

    # 检查文件是否存在
    if not os.path.exists(checkpoint_path):
        raise ValueError("Checkpoint file not found: {}".format(checkpoint_path))
    ...
    batch_size = min(batch_size, test_mains.shape[0])
...
def transfer(appliance_name, model, mains, appliance, epochs, batch_size, train_patience = 3):
    ...
    pretrained_dict = torch.load(os.path.join("./" + appliance_name + "_dae_best_05.pt"))
    ...
    for param in model.deconv_2.parameters():
    ...
    for epoch in range(epochs):
        ...
        if best_loss is None or final_loss < best_loss:
            best_loss = final_loss
            patience = 0
            best_model_path = "./{}_transfer_best_model.pth".format(appliance_name)
            torch.save(model.state_dict(), best_model_path)
        else:
            patience += 1

        print("Epoch: {}, Validation Loss: {}".format(epoch, final_loss))
...
class DAE(Disaggregator):    
    def __init__(self, params):
        self.MODEL_NAME = "DAE"
        self.sequence_length = params.get('sequence_length', None)
        self.n_epochs = params.get('n_epochs', None)
    ...
    def partial_fit(self, train_main, train_appliances, pretrain = False, do_preprocessing=True, pretrain_path = "./dae_pre_state_dict.pkl",**load_kwargs):
        ...
        for appliance_name, power in train_appliances:
            ...
            # transfer(appliance_name, model, train_main, power, self.n_epochs, self.batch_size, train_patience=3)
    ...
    def disaggregate_chunk(self, test_main_list, do_preprocessing = True):
        ...
        for test_main in test_main_list:
            ...
            for appliance in self.models:
                ...
                prediction = test(model, test_main, appliance)
